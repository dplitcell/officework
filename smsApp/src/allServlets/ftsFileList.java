	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(sessionId==null)	{
			...
			if (ftsType.matches("I"))
				sqlQuery="SELECT a.file_no, DECODE(a.cat_type, 'G', UPPER(a.subject), 'CONFIDENTIAL FILE') AS subject, CONCAT('SEND BY ', a.USER_NAME) AS CustField, 'RECEIVE' AS link_button, a.trn_id, d.session_id FROM V_FTS_FILE_STATUS a, x_sms_login d where a.TRN_TYPE= 'SEND' and trim(d.USER_ID) = trim(TO_CHAR(a.ref_user_code)) AND d.session_id = '1600922849933' ORDER BY a.trn_id DESC";
