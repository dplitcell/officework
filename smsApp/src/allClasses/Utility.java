...
*/	
	...
	public static List<ftsFile> GenerateFileRecord(String sqlQuery) {
		List<ftsFile> fileList = new ArrayList<ftsFile>();
		System.out.println("Query- "+sqlQuery);
    	Connection conn= new Utility().make_connection();
    	try{
    		ResultSet rs=conn.createStatement().executeQuery(sqlQuery);
    		while(rs.next()){
    			ftsFile file=new ftsFile();
    			file.setFile(rs.getString(1),rs.getString(2),rs.getString(3),rs.getString(4));
    			fileList.add(file);
    		}
    		conn.close();
    	}catch(SQLException ex){
	...
	public static void ftsCreate(String season, String ftsFileName, String ftsFileCat, String ftsFunction, String ftsFileNo, String ftsInitEmp, String userId) {
		Connection conn= new Utility().make_connection();
		try{
    		CallableStatement cstmt = conn.prepareCall("{CALL PCREATEFTSFILE(?,?,?,?,?,?,?)}");
    		cstmt.setString(1,season);
    		cstmt.setString(2,ftsFileName);
    		cstmt.setString(3,ftsFileCat);
    		cstmt.setString(4,ftsFunction);
    		cstmt.setString(5,ftsFileNo);
    		if (ftsInitEmp.isEmpty() || ftsInitEmp==null || ftsInitEmp == "")
    			cstmt.setString(6,userId);
    		else
    			cstmt.setString(6,ftsInitEmp);
	...
	public static void ftsSendFile(String season, String ftsFileNo, String ftsRefEmp, String ftsActivity) {
		Connection conn= new Utility().make_connection();
    	try{
    		CallableStatement cstmt = conn.prepareCall("{CALL PSENDFTSFILE(?,?,?,?)}");
    		cstmt.setString(1,season);
    		cstmt.setString(2,ftsFileNo);
    		cstmt.setString(3,ftsRefEmp);
    		cstmt.setString(4,ftsActivity);
    		cstmt.executeUpdate();
    		conn.close();
    	} catch(SQLException ex){
    		ex.printStackTrace();
	...
	public static void ftsAction(String season, String ftsAction, String ftsFileNo) {
		Connection conn= new Utility().make_connection();
    	try{
    		CallableStatement cstmt = conn.prepareCall("{CALL PUPDFTSFILE(?,?,?)}");
    		cstmt.setString(1,season);
    		cstmt.setString(2,ftsAction);
    		cstmt.setString(3,ftsFileNo);
    		cstmt.executeUpdate();
    		conn.close();
    	} catch(SQLException ex){
    		ex.printStackTrace();
	...
	public static List<Menu> GenerateMenu(String session, String module)
	{
    	...
    	String query="SELECT * FROM v_sms_menu WHERE session_id = '"+session+"' AND module_id = '" + module + "'";
    	Connection conn=new Utility().make_connection();
    	try
    	{
    		ResultSet rs=conn.createStatement().executeQuery(query);
...
//			Considered in the view
	...
	public static String verifyLogin(String module, String userid, String passwd) 
	{
    	...
    	try
    	{
    		...
    		CallableStatement cstmt = conn.prepareCall("{? = call fn_sms_verify_user(?,?,?)}");
 ...
 //   	System.out.println("Ret Rem:" + retFlag);
	...
	{
		String strReturn= " ";
		String query = "insert into x_sms_login (session_id, user_id, login_dt_tm, module_id) values ('"+session+"','"+userId+"',systimestamp, '" + module +"')";
		Connection conn=new Utility().make_connection();
    	try
    	{
    		conn.createStatement().executeUpdate(query);
    		query="select name from m_sms_user where user_id = '" + userId + "'";
    		ResultSet rs=conn.createStatement().executeQuery(query);
			while(rs.next())
			{
